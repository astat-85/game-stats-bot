...
class Database:
    ...
    """Экспорт в CSV с округлением чисел до 0.1 и выравниванием"""
    if not self.conn:
        self._connect()
    ...
    try:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8-sig') as f:
            ...
            for i, acc in enumerate(accounts, 1):
                ...
                # В CSV нельзя задать выравнивание, но данные подготовлены
                writer.writerow([
                    i,                          # № (число)
                    group,                       # Группа (число или пусто)
                    acc.get('game_nickname', ''),# Ник в игре (текст)
                    power,                       # Эл (число)
                    bm,                          # БМ (число)
                    pl1,                         # Пл 1 (число)
                    pl2,                         # Пл 2 (число)
                    pl3,                         # Пл 3 (число)
                    dragon,                      # Др (число)
                    buffs_stands,                 # БС (число)
                    buffs_research,               # БИ (число)
                    username,                     # ID имя (текст) - по левому краю в Excel
                    user_id,                      # ID номер (число)
                    time_str,                     # Время (текст)
                    date_str                      # Дата (текст)
                ])
